...
% point of MPECs.
% Implementation of a globally convergent method that computes B stationary points in MPECs.
% The approach is to solve an LPEC to make an active set guess for the complementarity variables,
% fix the active set and solve then to convergence a piece NLP.
% Variable decompistion,  x = (x0,x1,x2), where  0 \leq x1 \perp x2 \geq 0.
%
% Implemented and Developed by Armin Nurkanović, University of Freiburg, 2024.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TODO: settings.nlp_is_mpec = 1; check is this still used? (its purpose: if not an mpec, just make single nlp call without mpec machinery);
import casadi.*
%% Read input data
stats = struct;
t_prepare_mpec = tic;
[mpec_casadi, dims, solver_initalization, stats] =  create_mpec_functions(mpec,solver_initalization,settings);
% unfold_struct(mpec_casadi,'caller');
unfold_struct(solver_initalization,'caller');  % Todo: instead of unfold, use struct name;
...
switch settings.initialization_strategy
        ...
        if h_total_k <= settings.tol_feasibility
            ...
            if n_g > 0
                ...
                for ii = 1:n_g
                    ...
                    % lower bound;
                    if solver_initalization.ubg(ii)~= -inf
                        g_s = [g_s; mpec_casadi.g(ii)+s_ii];
                        lbg_s = [lbg_s; solver_initalization.lbg(ii)];
                        ubg_s = [ubg_s; inf];
                    end
                    % upper bound;
                    if solver_initalization.ubg(ii)~= inf
                        g_s = [g_s; mpec_casadi.g(ii)-s_ii];
                        lbg_s = [lbg_s; -inf];
                        ubg_s = [ubg_s; solver_initalization.ubg(ii)];
                    end
                end
                if settings.feasibility_project_to_bounds
                    x_projected = project_to_bounds(solver_initalization.x0,solver_initalization.lbx,solver_initalization.ubx,dims);
                    solver_initalization.x0 = x_projected;
                end
                s_max = full(mpec_casadi.h_std_fun(solver_initalization.x0,solver_initalization.p0))+1e-1;
                ...
                solver_initalization_feas = solver_initalization;
                solver_initalization_feas.lbg  = lbg_s;
                solver_initalization_feas.ubg = ubg_s;
                solver_initalization_feas.lbx = [s_lb;solver_initalization.lbx];
                solver_initalization_feas.ubx  = [s_ub; solver_initalization.ubx];
                solver_initalization_feas.x0 = [s0;solver_initalization.x0];
                [mpec_feas_casadi, dims_feas, solver_initalization_feas] =  create_mpec_functions(mpec_feas,solver_initalization_feas,settings);
                dims_feas.n_slacks = n_slacks;
                lpec_feas_casadi = create_lpec_functions(mpec_feas_casadi,dims_feas,settings,solver_initalization_feas);
                solver_initalization_feas.y_lpec_k_l = y_lpec_k_l;
                solver_initalization_feas.d_lpec_k_l = nan;
                phase_ii = false;
                [solution_feas,stats_feas] = mpecopt_phase_ii(mpec_feas_casadi,lpec_feas_casadi,dims_feas,settings,solver_initalization_feas,stats,phase_ii);
                ...
                if s_infeasiblity >= settings.tol_feasibility
                    ...
                    if full(mpec_casadi.h_comp_fun(x_k,solver_initalization.p0)) > 1e-3 && settings.debug_mode_on
                        keyboard;
                    end
                end
            else
    ...
    case 'TakeProvidedActiveSet'
        if isfield('y0',solver_initalization)
            if isequal(length(y0),dims.n_comp)
                y_lpec_k_l = solver_initalization.y0;
                I_plus_0 = y_lpec_k_l ==1;
                I_0_plus = y_lpec_k_l == 0;
            else
                warning('\n TakeProvidedActiveSet: lenth of y0 is %d, but required is %d',length(solver_initalization.y0))
                I_0_plus = x_k(dims.ind_x1)<=x_k(dims.ind_x2);
                I_plus_0 = x_k(dims.ind_x1)>x_k(dims.ind_x2);
            end
        else
            warning('\n TakeProvidedActiveSet as initalization strategy chosen, but no y0 provided. Resporting to TakeInitialGuessActiveSet')
            I_0_plus = x_k(dims.ind_x1)<=x_k(dims.ind_x2);
            I_plus_0 = x_k(dims.ind_x1)>x_k(dims.ind_x2);
        end
        lbx_bnlp_k = lbx;
...
solver_initalization.y_lpec_k_l = y_lpec_k_l;
solver_initalization.d_lpec_k_l = d_lpec_k_l;
phase_ii = true;
if settings.verbose_solver
    print_phase_ii();
    print_iter_header();
